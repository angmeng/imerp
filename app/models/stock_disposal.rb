class StockDisposal < ActiveRecord::Base
  belongs_to :user
  has_many :stock_disposal_items
  has_many :product_movements, :as => :movement
  
  validates :stock_disposal_date, :presence => true
  before_create :generate_sd_number
  ...
  def check_settle
      error_count = 0
      if stock_disposal_items.any?
        stock_disposal_items.each do |i|
...
def add_autocomplete_item(item)
     item_ids = item[:product_tokens].split(",")
      ...
      found = Product.find(i.to_i) rescue nil
       stock_disposal_items.create!(:product_id => i.to_i) if found
     end
  end
  ...
  def update_item(item)
    ...
    item.each do |item_id, content|
       i = stock_disposal_items.find(item_id.to_i)
        if content[:stock_location_id].to_i > 0
           if(content[:product_uom_id].to_i > 0)
             if stock_disposal_items.where("id != ? and product_id = ? and product_uom_id = ? and stock_location_id = ?", item_id, i.product_id, content[:product_uom_id],content[:stock_location_id].to_i ).first
  ...
  def remove_item(item)
    item.each do |inv_id, content|
      if content[:selected].to_i == 1
        i = stock_disposal_items.find(inv_id)
        i.destroy
      end
    end
 end
...
def void_status
 ...
 def post_quantity
    if posted?
      ...
      transaction do
        stock_disposal_items.each do |d|
          unless d.posted?
            ...
            item.description = ProductMovement::ADD_DISPOSAL
            item.save!
...
private
def generate_sd_number
    setting = Setting.first
    setting.stock_disposal_last_number += 1
    setting.save!
    self.stock_disposal_number = setting.stock_disposal_code + setting.stock_disposal_last_number.to_s
  end
end
