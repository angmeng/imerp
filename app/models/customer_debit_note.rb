class CustomerDebitNote < ActiveRecord::Base
  has_many :customer_debit_note_items
  has_many :product_movements, :as => :movement
  belongs_to :customer

   validates :cust_debit_note_date, :presence => true
  before_create :generate_cdn_number
  attr_reader :product_tokens

  
  def check_settle
      error_count = 0
      if customer_debit_note_items.any?
        customer_debit_note_items.each do |i|
  ...
  def update_item(item)
    ...
    item.each do |item_id, content|
      i = customer_debit_note_items.find(item_id.to_i)
      if content[:product_uom_id].to_i > 0
         if customer_debit_note_items.where("id != ? and product_id = ? and product_uom_id = ?", item_id, i.product_id, content[:product_uom_id]).first
 ...
 def remove_item(item)
    item.each do |inv_id, content|
      if content[:selected].to_i == 1
        i = customer_debit_note_items.find(inv_id)
        i.destroy
      end
    end
  end
  ...
  def add_autocomplete_item(item)
    ...
    item_ids.uniq.each do |i|
      ...
      customer_debit_note_items.create!(:product_id => i.to_i, :stock_location_id => 0) if found
    end
  ...
  def add_item(item)
    ...
    item.each do |p_id, content|
      if content[:selected].to_i == 1
        if content[:product_uom_id].to_i > 0
          if customer_debit_note_items.first(:conditions => ["product_id = ? and product_uom_id = ? ", p_id , content[:product_uom_id].to_i])
           error_msg << Product.find(p_id.to_i).code + " - " + ProductUom.find(content[:product_uom_id].to_i).name + " - is already exist <br>"
          else
              i = customer_debit_note_items.new
...
def quantity_post_status
  ...
  def post_quantity
    if posted?
      ...
      transaction do
        customer_debit_note_items.each do |d|
          ...
          unless d.posted?
            ...
            item.description = ProductMovement::ADD_CUSTOMER_DN_DESC
            item.save!
...
def remove_item(item)
    item.each do |item_id, content|
      if content[:selected]
        i = customer_debit_note_items.find(item_id)
        i.destroy
      end
    end
 end

def removed_cdn_item(item)
    item.each do |item_id, content|
      if content[:selected]
        i = customer_debit_note_items.find(item_id)
        i.destroy
 ...
 private
def generate_cdn_number
    setting = Setting.first
    setting.customer_debit_note_last_number += 1
    setting.save!
    self.cust_debit_note_number = setting.customer_debit_note_code + setting.customer_debit_note_last_number.to_s
  end
end
